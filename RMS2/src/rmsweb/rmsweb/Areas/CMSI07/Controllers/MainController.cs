...
namespace CMSI07.Controllers
{
    public class MainController : BaseController
    {
        ...
        public ActionResult Index(string action)
        {
            ...
            if (action == "btnExport")
            {
                ...
                lock (objLock)
                {
                    ...
                    using (FileStream fs = new FileStream(path + @"\CurrencyInfo.xls", FileMode.OpenOrCreate, FileAccess.ReadWrite))
                    {
                        ...
                        row.CreateCell(0).SetCellValue("幣別代號");
                        row.CreateCell(1).SetCellValue("幣別名稱");
                        row.CreateCell(2).SetCellValue("生效日期");
                        ...
                        for (int i = 0; i < dtExport.Rows.Count; i++)
                        {
                            ...
                            row.CreateCell(0).SetCellValue(dtExport.Rows[i]["CurrencyId"].ToString());
                            row.CreateCell(1).SetCellValue(dtExport.Rows[i]["Currency"].ToString());
                            row.CreateCell(2).SetCellValue(dtExport.Rows[i]["EffectiveDate"].ToString());
                            row.CreateCell(3).SetCellValue(dtExport.Rows[i]["ExchangeRate"].ToString());
                            index++;
                    ...
                    //return File(new FileStream(path + @"\CurrencyInfo.csv", FileMode.Open), "text/plain", "CurrencyInfo.csv");
                    return File(new FileStream(path + @"\CurrencyInfo.xls", FileMode.Open), "text/plain", "CurrencyInfo.xls");
                }
            }
        ...
        [HttpPost]
        public JsonResult CURMID(string currencyId)
        {
            CurrencyLogic logic = new CurrencyLogic();
            bool type = logic.IsModuleId(currencyId);
