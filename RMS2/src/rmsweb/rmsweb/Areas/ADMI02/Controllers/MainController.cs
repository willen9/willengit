    ...
    public class MainController : BaseController
    {
        ...
        public ActionResult Index(string action)
        {
            ...
            if (action == "btnSearch")
            {
                ...
                group.GroupId = Request.Form["GroupId"];
                DataTable dtDisplay = groupLogic.SelectGroup(group, Request.Form["selectCondition"], Request.Form["selectCondition1"]);
                ...
                foreach (DataRow dr in dtDisplay.Rows)
                {
                    ...
                    group.Remark = dr["Remark"].ToString();
                    lst.Add(group);
                }
                ViewBag.GroupId = Request.Form["GroupId"];
                ViewBag.GroupName = Request.Form["GroupName"];
            ...
            {
                GroupLogic groupLogic = new GroupLogic();
                DataTable dtDisplay = groupLogic.SelectGroup(null,null,null);
                List<Group> lst = new List<Group>();
                Group group = null;
                foreach (DataRow dr in dtDisplay.Rows)
                {
        ...
        {
            List<Group> lstAddress = new List<Group>();
            Group group=new Group();
            group.GroupId = Request.Form["GroupId"];
            group.GroupName= Request.Form["GroupName"];
            group.Remark= Request.Form["Remark"];

            GroupLogic groupLogic = new GroupLogic();
           //if (action == "btnEdit")
           // {
           //     DataTable dtDisplay = groupLogic.SelectModule(group, null, null);
           ...
           //     return View();
           // }
            if (Request.Form["action"] == "btnSaveNew" || Request.Form["action"] == "btnNextNew")
            {
                string msgInfo = "";
                string msg = "";
                group.ProgId = Request.Form["ProgId"];
                group.ProgName = Request.Form["ProgName"];
                group.ExcuteProg = Request.Form["ExcuteProg"];
                group.NewData = Request.Form["NewData"];
                group.ModiData = Request.Form["ModiData"];
                group.Confirm = Request.Form["Confirm"];
                group.Disconfirm = Request.Form["Disconfirm"];
                group.DelData = Request.Form["DelData"];
                group.Invalid = Request.Form["Invalid"];
                group.Print = Request.Form["Print"];
                group.Exprt = Request.Form["Export"];
                group.Cost = Request.Form["brand"];
                group.Price = Request.Form["brand"];

                if (!groupLogic.AddGroup(group, out msgInfo))
                {
                ...
                {
                    if (Request.Form["action"] == "btnSaveNew")
                    {
            ...
            //    if (!groupLogic.UpdateModule(group))
            //    {
            //        group.GroupId = Request.Form["GroupId"];
            //        group.GroupName = Request.Form["GroupName"];
            //        group.Remark = Request.Form["Remark"];
            ...
            {
                group.GroupId = id;
                DataTable dtDisplay = groupLogic.SelectGroup(group, null, null);
                if (dtDisplay.Rows.Count > 0)
                {
                    ViewBag.GroupId = dtDisplay.Rows[0]["GroupId"].ToString();
                    ViewBag.GroupName = dtDisplay.Rows[0]["GroupName"].ToString();
                    ViewBag.Remark = dtDisplay.Rows[0]["Remark"].ToString();
                }
            }
            else
            {
                //Select p.ProgId,p.ProgName,g.ExcuteProg,g.NewData,g.ModiData,g.Confirm,g.Disconfirm,g.DelData,g.Invalid,g.Print,g.Export,g.Cost,g.Price from Program as p left join GroupD as g on p.ProgId = g.ProgId
                ViewData["DisplayData"] = GetAddressInfo(group);
            }
        ...
        [HttpPost]
        public JsonResult CURMID(string groupId)
        {
            GroupLogic logic = new GroupLogic();
            bool type = logic.IsModuleId(groupId);
            return Json(type, JsonRequestBehavior.AllowGet);
        }

        private List<Group> GetAddressInfo(Group group)
        {
            GroupLogic groupLogic = new GroupLogic();
            Group tableListInfo = null;
            DataTable dtAddress = groupLogic.GetAddressInfo(group);
            List<Group> lst = new List<Group>();
            foreach (DataRow dr in dtAddress.Rows)
            {
                tableListInfo = new Group();
                tableListInfo.ProgId = dr["ProgId"].ToString();
                tableListInfo.ProgName = dr["ProgName"].ToString();
                tableListInfo.ExcuteProg = dr["ExcuteProg"].ToString();
                tableListInfo.NewData = dr["NewData"].ToString();
                tableListInfo.ModiData = dr["ModiData"].ToString();
                tableListInfo.Confirm = dr["Confirm"].ToString();
                tableListInfo.Disconfirm = dr["Disconfirm"].ToString();
                tableListInfo.DelData = dr["DelData"].ToString();
                tableListInfo.Invalid = dr["Invalid"].ToString();
                tableListInfo.Print = dr["Print"].ToString();
                tableListInfo.Exprt = dr["Export"].ToString();
                tableListInfo.Cost = dr["Cost"].ToString();
                tableListInfo.Price = dr["Price"].ToString();
