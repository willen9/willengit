                ...
                if (args.Length > 0)
                {
                    if (args[0].ToUpper() == "C") //轉檔
                    {
                        ...
                        DataTable dtExport = dataAccess.ExecuteDataTable(@"SELECT d.[MN-MachineType],d.[MN-NO],
                                                                            ...
                                                                            '0' AS ErrorCode
                                                                            FROM DASTA AS d
                                                                            WHERE d.[MN-STATES]='Pass'
                                                                            AND d.[MN-AIR_TEMP] BETWEEN '22' AND '28'
                                                                            AND d.[MN-MachineType] IN (SELECT d2.[MN-MachineType] FROM DASTA AS d2 WHERE d2.[MN-NO] LIKE '%GOLDEN' GROUP BY d2.[MN-MachineType] HAVING COUNT(*)>=6 )
                                                                            AND d.[MN-MachineType] IN 
                                                                            (
	                                                                            SELECT d3.[MN-MachineType]
	                                                                            FROM DASTA AS d3 
	                                                                            WHERE d3.[MN-NO] LIKE '%GOLDEN'
	                                                                            GROUP BY d3.[MN-MachineType] 
	                                                                            HAVING COUNT(*)>=6
	                                                                            AND  cast(MAX(d3.[MN-AIR_TEMP]) AS DECIMAL(8,4))- cast(MIN(d3.[MN-AIR_TEMP]) AS DECIMAL(8,4))<=3
                                                                            )
                                                                            AND d.[MN-DATE]+d.[MN-TIME] >=CONVERT(varchar(100),DATEADD(DAY,-1,GETDATE()), 111)+'07:00:00'
                                                                            AND d.[MN-DATE]+d.[MN-TIME] <CONVERT(varchar(100),GETDATE(), 111)+'07:00:00'
                        ...
                        foreach (DataRow dr in dtExport.Rows)
                        {
                            if (machineType == "")
                            {
                                ...
                                machineType = dr["MN-MachineType"].ToString();
                             
                                DataRow[] drs =
                               dtExport.Select("[MN-NO] like '%GOLDEN' AND [MN-MachineType]='" +
                                               dr["MN-MachineType"].ToString() + "'");
                            ...
                            else if (dr["MN-MachineType"].ToString() != machineType)
                            {
                                ...
                                if (dtHeader.Rows.Count > 0)
                                {
                                    ...
                                    row.GetCell(7).SetCellValue(dtHeader.Rows[0]["SOP005"].ToString());
                                    row.GetCell(9).SetCellValue(dtHeader.Rows[0]["SOP006"].ToString());
                                    row.GetCell(11).SetCellValue(dtHeader.Rows[0]["SOP002"].ToString());
